...
export default function QuickActions({ roomCode, isHost, userCount = 0 }) {
  ...
  function updateSetting(key) {
    ...
    const labels = {
      roomLocked: enabled ? "Oda gizlendi 🔒" : "Oda gösteriliyor 🔓",
      inviteOnly: enabled ? "Invite only açıldı 👥" : "Invite only kapandı 👥",
      muteAll: enabled ? "Herkes susturuldu 🔇" : "Ses serbest 🎤",
      chatLocked: enabled ? "Chat kilitlendi 💬" : "Chat açıldı 💬",
    };
  ...
  if (isHost) {
    items.push(
      {
        icon: Lock,
        label: roomSettings.roomLocked ? "Odayı Göster" : "Odayı Gizle",
        value: roomSettings.roomLocked ? "Gizli" : "Public",
      ...
      },
      {
        icon: UserCheck,
        label: "Invite Only",
        value: roomSettings.inviteOnly ? "Davet şart" : "Açık giriş",
      ...
      },
      {
        icon: MicOff,
        label: "Mute All",
        value: roomSettings.muteAll ? "Herkes sessiz" : "Serbest",
      ...
      },
      {
        ...
        label: "Chat Lock",
        value: roomSettings.chatLocked ? "Kilitli" : "Açık",
