      ...
      <div className="-mt-10 p-4 sm:-mt-8">
        ...
        <div className="mt-4 min-w-0">
            ...
            <div className="mt-4 overflow-hidden rounded-[1.75rem] border border-emerald-300/15 bg-gradient-to-br from-emerald-400/15 to-sky-400/10 p-4 shadow-[0_18px_70px_rgba(16,185,129,0.08)]">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-emerald-200/70"><PlayCircle size={14} /> Continue Watching</div>
                  <p className="mt-3 line-clamp-2 text-base font-black leading-5 text-white sm:truncate sm:text-sm">{continueWatching.title || continueWatching.url}</p>
                  <p className="mt-2 text-xs font-bold text-white/50">⏱ {continueTime > 5 ? formatPlaybackTime(continueTime) : "Başlangıç"}{continueWatching.roomCode || continueWatching.meta ? ` • ${continueWatching.roomCode || continueWatching.meta}` : ""}</p>
                </div>
                <button type="button" onClick={() => onResumeWatch?.(continueWatching)} className="shrink-0 rounded-2xl bg-emerald-400/20 px-4 py-3 text-xs font-black text-emerald-100 transition hover:bg-emerald-400/30 active:scale-95">Devam Et</button>
              </div>
            </div>
          ) : null}
          ...
          <div className="mt-4 rounded-3xl border border-sky-300/10 bg-sky-400/5 p-3">
            <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-sky-200/55"><Sparkles size={13} /> Favorites</div>
            <div className="flex flex-wrap gap-2">
              {platforms.map((platform) => (
                <span key={platform} className="inline-flex items-center gap-1 rounded-full bg-white/8 px-3 py-1 text-xs font-black text-white/65">{platform.toLowerCase().includes("youtube") ? <Film size={12} /> : <Clock3 size={12} />}{platform}</span>
              ))}
            </div>
          </div>
          ...
          <div className="mt-4 rounded-3xl border border-white/8 bg-white/[0.03] p-3">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-white/35"><Clock3 size={13} /> Watch History</div>
              {hiddenHistoryCount > 0 ? <button type="button" onClick={() => setHistoryOpen((value) => !value)} className="rounded-full bg-white/8 px-3 py-1 text-[11px] font-black text-white/55 transition hover:bg-white/12 hover:text-white">{historyOpen ? "Kısalt" : `+${hiddenHistoryCount} daha`}</button> : null}
            </div>
            <WatchHistory items={compactHistory} stats={stats} onResumeWatch={onResumeWatch} />
          </div>
        </div>
      </div>
    </section>
