import { Home, LogOut, Menu, MessageSquare, Settings, UserRound, UsersRound } from "lucide-react";
...
export default function VorySidebar({
  ...
  return (
    <>
      <aside className={`vory-v5-sidebar !rounded-[2rem] ${performanceMode ? "!bg-black/70" : "!bg-black/25 !backdrop-blur-2xl"}`}>
        <button
          type="button"
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-[9999] bg-black/65" onClick={() => setDrawerOpen(false)}>
          <aside
            className={`h-full w-[min(420px,82vw)] overflow-auto border-r border-white/10 bg-[#09090d]/92 px-7 py-8 ${performanceMode ? "shadow-[20px_0_60px_rgba(0,0,0,0.42)]" : "shadow-[30px_0_100px_rgba(0,0,0,0.55)] backdrop-blur-2xl"}`}
          ...
          >
            ...
            <div className="space-y-8">
              ...
              <button
                ...
                className="flex w-full flex-col items-center gap-3 rounded-[2rem] py-4 text-white transition hover:bg-white/8"
              >
                <span className="flex h-24 w-24 items-center justify-center rounded-full bg-fuchsia-500/14 text-fuchsia-100 shadow-[inset_0_0_0_1px_rgba(217,70,239,0.18)]">
                  <MessageSquare size={44} />
                </span>
                <span className="text-3xl font-black">Geri Bildirim</span>
              </button>
