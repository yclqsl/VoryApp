...
function getActivityTimeText(createdAt) {
  ...
  if (diffSeconds < 15) return isOnline ? "şimdi" : "az önce";
...
function getLastSeenText(updatedAt, isOnline, lastSeenAt) {
  const targetTime = Number(isOnline ? updatedAt : (lastSeenAt || updatedAt));

  if (!targetTime) return isOnline ? "şimdi" : "son görülme yok";
...
function buildFriendActivity({ friends = [], onlineUsers = [], currentSocketId }) {
  ...
  return (friends || [])
    .map((friend) => {
      ...
      const onlineActive = !!online?.socketId && online?.connected !== false && online?.isOnline !== false;
      ...
      return {
        ...
        lastSeenAt: online?.lastSeenAt || online?.updatedAt || friend?.lastSeenAt || friend?.updatedAt || "",
      };
    })
    .sort((a, b) => {
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-bold text-white/55">
                      ...
                      <span className="inline-flex items-center gap-1 text-white/35">
                        <Clock size={12} />
                        {getLastSeenText(user.updatedAt, user.isOnline, user.lastSeenAt)}
                      </span>
                    </div>

                    {user.activity === "watching" || Number(user.watchTime || 0) > 0 ? (
