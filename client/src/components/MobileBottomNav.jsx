...
const tabs = [
  { id: "watch", label: "Party", icon: Home },
  { id: "people", label: "Crew", icon: UsersRound },
  { id: "profile", label: "Profile", icon: UserRound },
  { id: "room", label: "Controls", icon: Radio },
];
...
export default function MobileBottomNav({
  ...
  return (
    <nav className="vory-mobile-bottom-nav vory-rave-gap-mobile-nav fixed inset-x-4 bottom-4 z-50 rounded-[2rem] border border-white/10 bg-black/82 p-2 shadow-[0_24px_90px_rgba(0,0,0,0.6)] backdrop-blur-2xl lg:hidden">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              type="button"
              onClick={() => tab.id === "room" && !roomCode ? onChange?.("watch") : onChange?.(tab.id)}
              className={`relative flex flex-col items-center justify-center gap-1 rounded-[1.4rem] px-2 py-2.5 text-[10px] font-black transition ${tab.id === "room" && !roomCode ? "opacity-55 " : ""}${
                active
